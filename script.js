...
function limpiarAnalisis() {
  ...
  // Limpiar el filtro de municipios
  if (typeof limpiarFiltroMunicipio === 'function') {
...
async function cargarCapa(nombre) {
  ...
  try {
    ...
    if (!data) {
      ...
      // Actualizar selector de municipios si se cargó la capa de municipios
      if (nombre === 'municipios' || nombre === 'municipios_geojson') {
        setTimeout(() => {
          if (typeof poblarSelectorMunicipios === 'function') {
    ...
    if (nombre === 'municipios' || nombre === 'municipios_geojson') {
      const geoJsonLayer = L.geoJSON(null, {
        ...
        onEachFeature: (feature, layer) => {
          ...
          // Obtener el nombre del municipio del campo nomgeo
          const nombreMunicipio = props.nomgeo || props.NOMGEO || props.NomGeo || props.municipi_1 || 'Sin nombre';
          ...
          popup += `<b>Municipio:</b> ${nombreMunicipio}<br>`;
          Object.keys(props).forEach(key => {
            if (key !== 'geom' && key !== 'municipi_1' && key !== 'nomgeo' && key !== 'NOMGEO' && key !== 'NomGeo') {
...
// ============================================================================
// FILTRO POR MUNICIPIO - CONTEO DE INFRAESTRUCTURA CAEM
// ============================================================================

// Variable global para almacenar el polígono del municipio seleccionado
let municipioSeleccionadoPolygon = null;
let municipioHighlightLayer = null;

// Capas del Inventario CAEM que se deben contar (excluyendo líneas)
const capasInventarioParaConteo = [
  'cajas de captacion',
  'cajas derivadoras', 
  'cajas rompedoras de presion',
  'campamentos_edomex',
...
// Capas del Programa Operativo Anual 2025
const capasPOAParaConteo = [
  'caem-dgig-fise-052-25-cp',
  'caem-dgig-fise-053-25-cp',
  'caem-dgig-fise-054-25-cp',
  'caem-dgig-fise-055-25-cp',
...
// Capas de Inundaciones (Atlas)
const capasInundacionesParaConteo = [
  'atlas temporada 2020',
  'atlas temporada 2021',
  'atlas temporada 2022',
  'atlas temporada 2023',
...
// Función para poblar el selector de municipios
function poblarSelectorMunicipios() {
  const select = document.getElementById('municipio-select');
  if (!select) return;
  
  // Buscar datos de municipios
  const municipiosData = capasData['municipios'] || capasData['municipios_geojson'];
  
  if (!municipiosData || municipiosData.length === 0) {
    console.log('⚠️ No hay datos de municipios cargados');
    return;
  }
  
  // Extraer nombres únicos de municipios y ordenarlos
  const municipios = [...new Set(municipiosData.map(d => {
    return d.municipi_1 || d.MUNICIPI_1 || d.nomgeo || d.NOMGEO || 'Sin Nombre';
  }))].filter(m => m && m !== 'Sin Nombre').sort();
...
// Función principal para filtrar por municipio
function filtrarPorMunicipio() {
  const select = document.getElementById('municipio-select');
  const statsContainer = document.getElementById('municipio-stats');
  const municipioNombre = select.value;
  
  if (!municipioNombre) {
    statsContainer.style.display = 'none';
    limpiarHighlightMunicipio();
    return;
  }
  
  // Mostrar indicador de carga
  statsContainer.style.display = 'block';
  document.getElementById('municipio-nombre-selected').textContent = municipioNombre;
  document.getElementById('municipio-stats-grid').innerHTML = '<div class="municipio-loading" style="grid-column: span 2;">Calculando...</div>';
  document.getElementById('municipio-area-value').textContent = '...';
  
  // Procesar de forma asíncrona para no bloquear la UI
  setTimeout(() => {
    procesarFiltroMunicipio(municipioNombre);
  }, 100);
}

// Procesar el filtro de municipio
function procesarFiltroMunicipio(municipioNombre) {
  const municipiosData = capasData['municipios'] || capasData['municipios_geojson'];
  
  if (!municipiosData) {
    console.error('No hay datos de municipios');
    return;
  }
  
  // Encontrar el municipio seleccionado
  const municipioFeature = municipiosData.find(d => {
    const nombre = d.municipi_1 || d.MUNICIPI_1 || d.nomgeo || d.NOMGEO;
    return nombre === municipioNombre;
  });
  
  if (!municipioFeature) {
    console.error('Municipio no encontrado:', municipioNombre);
    return;
  }
  
  // Obtener el área del campo km2 de los datos
  let areaKm2 = parseFloat(municipioFeature.km2 || municipioFeature.KM2 || municipioFeature.area_km2 || 0);
  
  // Crear polígono del municipio y reproyectar
  let municipioPolygon;
  let geometryForMap;
  try {
    let geom;
    if (municipioFeature.geom) {
      // Si viene de PostGIS, puede ser string JSON
      geom = typeof municipioFeature.geom === 'string' ? JSON.parse(municipioFeature.geom) : municipioFeature.geom;
    } else if (municipioFeature.geometry) {
      geom = municipioFeature.geometry;
    }
    
    if (!geom) {
      console.error('No se encontró geometría para el municipio');
      return;
    }
    
    // Reproyectar la geometría de UTM a WGS84
    geometryForMap = reprojectGeometry(geom);
    
    // Crear feature para Turf (para operaciones espaciales)
    municipioPolygon = turf.feature(geometryForMap);
    
  } catch (e) {
    console.error('Error procesando geometría del municipio:', e);
    return;
  }
  
  municipioSeleccionadoPolygon = municipioPolygon;
  
  // Resaltar el municipio en el mapa y activar capa de municipios
  resaltarMunicipio(geometryForMap, municipioNombre);
  
  // Contar elementos de cada capa del Inventario CAEM
  const conteosInventario = {};
  
  capasInventarioParaConteo.forEach(capaKey => {
    const capaData = capasData[capaKey];
    if (!capaData || capaData.length === 0) {
      conteosInventario[capaKey] = 0;
      return;
    }
    
    let count = 0;
    
    capaData.forEach(feature => {
      try {
        let geom = feature.geom || feature.geometry;
        if (!geom) return;
        ...
        const geomReproyectada = reprojectGeometry(geom);
        
        // Verificar tipo de geometría (solo puntos y polígonos)
        const geomType = geomReproyectada.type;
        if (!geomType) return;
        
        // Excluir líneas
        if (geomType.includes('Line') || geomType.includes('line')) return;
        
        // Crear feature de Turf
        const turfFeature = turf.feature(geomReproyectada);
        
        // Verificar si está dentro del municipio
        if (geomType === 'Point' || geomType === 'point') {
          if (turf.booleanPointInPolygon(turfFeature, municipioPolygon)) {
            count++;
          }
        } else if (geomType.includes('Polygon') || geomType.includes('polygon')) {
          try {
            if (turf.booleanIntersects(turfFeature, municipioPolygon)) {
              count++;
            }
          } catch (e) {
            try {
              const centroid = turf.centroid(turfFeature);
              if (turf.booleanPointInPolygon(centroid, municipioPolygon)) {
                count++;
              }
            } catch (e2) {}
          }
    ...
    conteosInventario[capaKey] = count;
  });
  
  // Contar elementos del POA (con campo Contrato)
  const conteosPOA = {};
  
  capasPOAParaConteo.forEach(capaKey => {
    const capaData = capasData[capaKey];
    if (!capaData || capaData.length === 0) {
      return;
    }
    ...
    capaData.forEach(feature => {
      try {
        ...
        if (geomType === 'Point' || geomType === 'point') {
          dentroMunicipio = turf.booleanPointInPolygon(turfFeature, municipioPolygon);
        } else {
          try {
            dentroMunicipio = turf.booleanIntersects(turfFeature, municipioPolygon);
          } catch (e) {
            try {
              const centroid = turf.centroid(turfFeature);
              dentroMunicipio = turf.booleanPointInPolygon(centroid, municipioPolygon);
            } catch (e2) {}
          }
        }
        
        if (dentroMunicipio) {
          // Obtener el campo Contrato
          const contrato = feature.Contrato || feature.contrato || feature.CONTRATO || 'Sin Contrato';
          if (!conteosPOA[contrato]) {
            conteosPOA[contrato] = { count: 0, capa: capaKey };
          }
          conteosPOA[contrato].count++;
        }
      } catch (e) {
        // Ignorar errores
      }
  ...
  // Contar elementos de Inundaciones (por año)
  const conteosInundaciones = {};
  
  capasInundacionesParaConteo.forEach(capaKey => {
    const capaData = capasData[capaKey];
    if (!capaData || capaData.length === 0) {
      return;
    }
    
    // Extraer el año del nombre de la capa
    const match = capaKey.match(/\d{4}/);
    const año = match ? match[0] : capaKey;
    ...
    if (count > 0) {
      conteosInundaciones[año] = count;
    }
  });
  
  // Mostrar resultados
  mostrarEstadisticasMunicipio(conteosInventario, conteosPOA, conteosInundaciones, areaKm2, municipioNombre);
}

// Función para mostrar las estadísticas
function mostrarEstadisticasMunicipio(conteosInventario, conteosPOA, conteosInundaciones, areaKm2, municipioNombre) {
  const statsGrid = document.getElementById('municipio-stats-grid');
  const areaValue = document.getElementById('municipio-area-value');
  
  // Generar HTML para las estadísticas
  let html = '';
  
  // Sección Inventario CAEM
  const capasInventarioConElementos = Object.entries(conteosInventario)
    .filter(([key, value]) => value > 0)
    .sort((a, b) => b[1] - a[1]);
  
  if (capasInventarioConElementos.length > 0) {
    html += '<div class="municipio-stats-section-title">Inventario CAEM</div>';
    capasInventarioConElementos.forEach(([capaKey, count]) => {
      const nombreDisplay = nombresCapas[capaKey] || capaKey;
      html += `
        <div class="municipio-stat-box">
          <div class="municipio-stat-label">${nombreDisplay}</div>
          <div class="municipio-stat-value">${count.toLocaleString()}</div>
        </div>
      `;
    });
  }
  
  // Sección Inundaciones (ahora antes de POA)
  const añosInundaciones = Object.entries(conteosInundaciones)
    .filter(([key, value]) => value > 0)
    .sort((a, b) => a[0].localeCompare(b[0])); // Ordenar por año ascendente
  
  if (añosInundaciones.length > 0) {
    html += '<div class="municipio-stats-section-title">Inundaciones</div>';
    añosInundaciones.forEach(([año, count]) => {
      html += `
        <div class="municipio-stat-box municipio-stat-inundaciones">
          <div class="municipio-stat-label">${año}</div>
          <div class="municipio-stat-value">${count.toLocaleString()}</div>
        </div>
      `;
    });
  ...
  // Sección POA (Programa Operativo Anual 2025) - Ahora al final como lista
  const contratosPOA = Object.entries(conteosPOA)
    .filter(([key, value]) => value && value.count > 0)
    .sort((a, b) => a[0].localeCompare(b[0])); // Ordenar alfabéticamente
  
  if (contratosPOA.length > 0) {
    html += '<div class="municipio-stats-section-title">Programa Operativo Anual (2025)</div>';
    html += '<ul class="municipio-poa-list">';
    contratosPOA.forEach(([contrato, info]) => {
      const capaKey = info.capa || '';
      html += `
        <li class="municipio-poa-item" onclick="zoomToContratoPOA('${capaKey}', '${contrato}')" title="Clic para ver en el mapa">
          <svg class="municipio-poa-item-icon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path d="M12 7V3H2v18h20V7H12zM6 19H4v-2h2v2zm0-4H4v-2h2v2zm0-4H4V9h2v2zm0-4H4V5h2v2zm4 12H8v-2h2v2zm0-4H8v-2h2v2zm0-4H8V9h2v2zm0-4H8V5h2v2zm10 12h-8v-2h2v-2h-2v-2h2v-2h-2V9h8v10zm-2-8h-2v2h2v-2zm0 4h-2v2h2v-2z"/>
          </svg>
          <span class="municipio-poa-item-text">${contrato}</span>
        </li>
      `;
    });
    html += '</ul>';
  ...
  // Si no hay nada
  if (html === '') {
    html = '<div style="grid-column: span 3; text-align: center; color: #666; font-size: 10px; padding: 10px;">No se encontraron elementos en este municipio</div>';
  }
  
  statsGrid.innerHTML = html;
  
  // Mostrar área con formato de miles (comas)
  const areaFormateada = areaKm2.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  areaValue.textContent = areaFormateada + ' km²';
}

// Función para resaltar el municipio en el mapa
function resaltarMunicipio(geometryForMap, municipioNombre) {
  // Limpiar highlight anterior
  limpiarHighlightMunicipio();
  
  try {
    // Activar la capa de municipios si no está activa
    activarCapaMunicipios();
    
    // Crear capa de resaltado con la geometría ya reproyectada
    const geoJsonData = {
      type: 'Feature',
      properties: { nombre: municipioNombre },
      geometry: geometryForMap
    };
    
    municipioHighlightLayer = L.geoJSON(geoJsonData, {
      style: {
        color: '#8a2035',
        weight: 4,
        fillColor: '#b99056',
    ...
    // Hacer zoom al municipio con verificación de bounds válidos
    const bounds = municipioHighlightLayer.getBounds();
    
    // Verificar que los bounds sean válidos (no infinitos ni muy pequeños)
    if (bounds.isValid()) {
      const ne = bounds.getNorthEast();
      const sw = bounds.getSouthWest();
      
      // Verificar que las coordenadas estén en rangos válidos para México
      if (ne.lat > 10 && ne.lat < 35 && ne.lng > -120 && ne.lng < -85 &&
          sw.lat > 10 && sw.lat < 35 && sw.lng > -120 && sw.lng < -85) {
        map.fitBounds(bounds, { 
          padding: [50, 50],
          maxZoom: 14
        });
      } else {
        console.warn('Bounds fuera de rango para México, usando centroide');
        // Calcular centroide y hacer zoom manual
        const centerLat = (ne.lat + sw.lat) / 2;
        const centerLng = (ne.lng + sw.lng) / 2;
        map.setView([centerLat, centerLng], 11);
      }
    } else {
      console.error('Bounds inválidos para el municipio');
    }
  } catch (e) {
    console.error('Error resaltando municipio:', e);
  }
}

// Función para activar la capa de municipios
function activarCapaMunicipios() {
  // Buscar el checkbox de municipios y activarlo si no está activo
  const checkboxMunicipios = document.querySelector('input[data-layer="municipios"]');
  if (checkboxMunicipios && !checkboxMunicipios.checked) {
    checkboxMunicipios.checked = true;
    // Disparar el evento change para que se cargue la capa
    checkboxMunicipios.dispatchEvent(new Event('change'));
    console.log('✅ Capa de municipios activada');
  }
}

// Función para hacer zoom a un contrato del POA
function zoomToContratoPOA(capaKey, contrato) {
  console.log(`🔍 Buscando contrato "${contrato}" en capa "${capaKey}"`);
  
  if (!capaKey) {
    console.warn('No se especificó la capa del contrato');
    return;
  }
  
  // Expandir el menú de Programa Operativo Anual y activar la capa
  expandirMenuPOA(capaKey);
  
  // Activar la capa si no está activa
  const checkbox = document.querySelector(`input[data-layer="${capaKey}"]`);
  if (checkbox && !checkbox.checked) {
    checkbox.checked = true;
    checkbox.dispatchEvent(new Event('change'));
    console.log(`✅ Capa ${capaKey} activada`);
  }
  
  // Esperar un momento para que la capa se cargue
  setTimeout(() => {
    // Buscar la capa activa
    const layer = capasActivas[capaKey];
    if (!layer) {
      console.warn(`Capa ${capaKey} no encontrada en capas activas`);
      return;
    }
    
    // Buscar el feature con el contrato especificado
    ...
    layer.eachLayer(function(featureLayer) {
      if (featureLayer.feature && featureLayer.feature.properties) {
        ...
        const featureContrato = props.Contrato || props.contrato || props.CONTRATO;
        if (featureContrato === contrato) {
          foundFeatures.push(featureLayer);
          if (featureLayer.getBounds) {
            if (!targetBounds) {
              targetBounds = featureLayer.getBounds();
            } else {
              targetBounds.extend(featureLayer.getBounds());
            }
          } else if (featureLayer.getLatLng) {
            const latlng = featureLayer.getLatLng();
            if (!targetBounds) {
              targetBounds = L.latLngBounds([latlng, latlng]);
            } else {
              targetBounds.extend(latlng);
            }
          }
        }
      }
    ...
    if (targetBounds && targetBounds.isValid()) {
      ...
      console.log(`✅ Zoom realizado al contrato "${contrato}" (${foundFeatures.length} elementos encontrados)`);
      
      // Resaltar los features encontrados brevemente (color dorado)
      foundFeatures.forEach(fl => {
        if (fl.setStyle) {
          const originalStyle = fl.options;
          fl.setStyle({ 
            color: '#B99056', 
            weight: 5, 
            fillOpacity: 0.5 
          ...
          setTimeout(() => {
            if (fl.setStyle && originalStyle) {
              fl.setStyle(originalStyle);
            }
          }, 2000);
        }
      });
    } else {
      console.warn(`No se encontraron elementos con contrato "${contrato}"`);
    }
  }, 500);
}

// Función para expandir el menú de Programa Operativo Anual
function expandirMenuPOA(capaKey) {
  // Buscar el checkbox de la capa
  const checkbox = document.querySelector(`input[data-layer="${capaKey}"]`);
  if (!checkbox) {
    console.warn('No se encontró el checkbox de la capa');
    return;
  }
  
  // Encontrar el contenedor padre y expandir todos los grupos colapsados
  let element = checkbox.parentElement;
  while (element) {
    // Si es un grupo colapsado, expandirlo
    if (element.classList && element.classList.contains('layers-group-content') && element.classList.contains('collapsed')) {
      element.classList.remove('collapsed');
      // También actualizar el ícono toggle del grupo
      const parentGroup = element.parentElement;
      if (parentGroup) {
        const toggleIcon = parentGroup.querySelector('.layers-group-toggle');
  ...
  console.log('✅ Menú de POA expandido');
}

// Función para limpiar el highlight del municipio
function limpiarHighlightMunicipio() {
...
function limpiarFiltroMunicipio() {
  ...
  select.value = '';
  statsContainer.style.display = 'none';
  municipioSeleccionadoPolygon = null;
  limpiarHighlightMunicipio();
  
  // Desactivar la capa de municipios
  desactivarCapaMunicipios();
}

// Función para desactivar la capa de municipios
function desactivarCapaMunicipios() {
  const checkboxMunicipios = document.querySelector('input[data-layer="municipios"]');
  if (checkboxMunicipios && checkboxMunicipios.checked) {
    checkboxMunicipios.checked = false;
    checkboxMunicipios.dispatchEvent(new Event('change'));
    console.log('✅ Capa de municipios desactivada');
  }
}

// Función para actualizar el selector cuando se cargan nuevas capas
function actualizarSelectorMunicipios() {
  // Solo poblar si hay datos de municipios
  if (capasData['municipios'] || capasData['municipios_geojson']) {
    poblarSelectorMunicipios();
  }
}

// Función para cargar datos de municipios en segundo plano (sin mostrar en mapa)
async function cargarMunicipiosParaFiltro() {
  // Si ya hay datos cargados, solo poblar el selector
  if (capasData['municipios'] && capasData['municipios'].length > 0) {
    console.log('✅ Datos de municipios ya cargados, poblando selector...');
    poblarSelectorMunicipios();
    return;
  }
  
  // Obtener configuración de Supabase
  const url = document.getElementById('url')?.value || 'https://ppdpjvfpujjfbwpuifmi.supabase.co';
  const key = document.getElementById('key')?.value || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBwZHBqdmZwdWpqZmJ3cHVpZm1pIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjA1MDAzMDksImV4cCI6MjA3NjA3NjMwOX0.2Pm_217ZLaYS-W8fDyE7bEr0IP0Y-fNZwVkuboBRRDo';
  
  console.log('🔄 Cargando datos de municipios para el filtro...');
  
  try {
    // Cargar todos los datos de municipios
    const municipiosData = await cargarCapaEnSegundoPlano('municipios', url, key);
    if (municipiosData && municipiosData.length > 0) {
      capasData['municipios'] = municipiosData;
      console.log(`✅ ${municipiosData.length} municipios cargados para el filtro`);
      poblarSelectorMunicipios();
    }
  } catch (error) {
...
// Función genérica para cargar una capa en segundo plano
async function cargarCapaEnSegundoPlano(nombreCapa, url, key) {
  try {
    let allData = [];
    let offset = 0;
    const limit = 1000;
    ...
    const encodedTable = encodeURIComponent(nombreCapa);
    
    while (hasMore) {
      const response = await fetch(
        `${url}/rest/v1/${encodedTable}?select=*&offset=${offset}&limit=${limit}`,
        {
          headers: {
            'apikey': key,
...
// Función para cargar todas las capas del Inventario CAEM en segundo plano
async function cargarCapasInventarioParaFiltro() {
  const url = document.getElementById('url')?.value || 'https://ppdpjvfpujjfbwpuifmi.supabase.co';
  const key = document.getElementById('key')?.value || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBwZHBqdmZwdWpqZmJ3cHVpZm1pIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjA1MDAzMDksImV4cCI6MjA3NjA3NjMwOX0.2Pm_217ZLaYS-W8fDyE7bEr0IP0Y-fNZwVkuboBRRDo';
  
  console.log('🔄 Cargando capas para el filtro de municipios...');
  
  // Combinar todas las capas a cargar
  const todasLasCapas = [
    ...capasInventarioParaConteo,
    ...capasPOAParaConteo,
    ...capasInundacionesParaConteo
  ];
  
  // Cargar cada capa en paralelo
  const promesas = todasLasCapas.map(async (nombreCapa) => {
    // Solo cargar si no hay datos ya
    if (!capasData[nombreCapa] || capasData[nombreCapa].length === 0) {
      const data = await cargarCapaEnSegundoPlano(nombreCapa, url, key);
      if (data && data.length > 0) {
        capasData[nombreCapa] = data;
        console.log(`  ✅ ${nombreCapa}: ${data.length} registros`);
      }
    } else {
      console.log(`  ✓ ${nombreCapa}: ya cargada (${capasData[nombreCapa].length} registros)`);
    }
  });
  
  await Promise.all(promesas);
  console.log('✅ Todas las capas cargadas para el filtro de municipios');
}

// Cargar municipios y capas del inventario automáticamente después de que se conecte a Supabase
document.addEventListener('DOMContentLoaded', function() {
  // Esperar a que se complete la conexión inicial y luego cargar datos
  setTimeout(async () => {
    await cargarMunicipiosParaFiltro();
    // Después de cargar municipios, cargar las demás capas
    setTimeout(() => {
      cargarCapasInventarioParaFiltro();
    }, 1000);
  }, 3000);
});

// Intentar poblar el selector periódicamente hasta que haya datos
let municipioCheckInterval = setInterval(() => {
  if (capasData && (capasData['municipios'] || capasData['municipios_geojson'])) {
    poblarSelectorMunicipios();
    clearInterval(municipioCheckInterval);
  }
}, 2000);
...
console.log('✅ Sistema de filtro por municipio cargado');
